---
name: new-worktree
description: Create an isolated git worktree with a new branch from the freshly fetched default branch, for starting feature development without disturbing unrelated checkouts or local base-branch edits. Use when the user says "/new-worktree", "new worktree", "gitworktree", "create a worktree", "start a feature branch in a separate checkout", "begin a new feature without touching this branch", or asks for a new branch where parallel development or preserving the current worktree matters.
---
...
Start a new feature branch in a separate git worktree based on the freshly fetched default branch (`origin/HEAD`, falling back to `main`). Leave local branches, the local base branch, unrelated worktrees, and uncommitted changes untouched. Do not require the current worktree to be clean.
...
```bash
SKILL_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/new-worktree"
bash "$SKILL_DIR/scripts/start_feature_worktree.sh" feat/my-feature
```

Optional arguments:

```bash
bash "$SKILL_DIR/scripts/start_feature_worktree.sh" feat/my-feature --path /absolute/worktree/path
```
...
- Resolves the base branch from `origin/HEAD`, then from `git ls-remote --symref origin HEAD`, and only then falls back to `main`.
- Fetches that branch directly into `refs/remotes/origin/<base>` immediately before creating the worktree.
- Creates the feature branch from the freshly fetched remote base, not its local counterpart or the current branch.
- Does not check out, pull, fast-forward, stash, run `git status`, or otherwise modify the local base branch; dirty local worktrees are allowed.
...
- Never require a clean current worktree; a dirty local base branch must not block creating the new worktree.
- Never check out, pull, fast-forward, or modify the local base branch.
- Never use the current branch or a local branch as the start point — only the freshly fetched `origin/<base>`.
- Never skip fetching the base branch immediately before creating the worktree.
- Never assume the base is `main` — resolve the repository's default branch, and fall back to `main` only when it cannot be determined.
